...
Для функционирования программы необходимы файлы, список которых изображен на рисунке слева. При отсутствии какого-либо файла работа программы не гарантируется. Файл settings.js – конфигурационный файл программы. Файлы *.dll – библиотеки, использующиеся для подключения к БАЗЕ и расшифровки сообщений от КЛИЕНТОВ. Файл MailSender.exe – программа отправки почты. Когда лимит аккаунтов достигнут, то СЕРВЕР должен уведомить об этом. СЕРВЕР запускает программу отправки почты и передает ей параметры: указанный лимит и название объекта. Программа MailSender.exe берет из файла настроек массив с перечисленными адресами-получателями и осуществляет рассылку.
...
* DatabaseName - имя локальной базы данных SQLite
* PostgresConnectionString – строка для подключения к Postgres. Не рекомендуется менять. 
* CenterName – название центра. 
* AskAccounts – true: сервер при достижении лимита будет запрашивать аккаунты у других; false - не будет. 
* AddressesList – список внешних IP-адресов других центров.
* GiveAccounts – true - сервер будет выдавать аккаунты по запросу.
* SetIssueLimit – true - установлен искусственный лимит выдачи аккаунтов внутри центра. Свыше значения IssueLimitValue аккаунты клиентам выдаваться не будут. 
* UsedMinuteLimit – время, после которого аккаунт освобождается принудительно в случае если от него за это время не было сообщений. 


СЕРВЕР располагает окном настроек, где можно изменять настройки, не открывая settings.js. 
...
* NoAvailableAccountsMessage – отображается, когда от СЕРВЕРА пришел ответ об отсутствии свободных аккаунтов
* BadConnectionMessage – сообщение при отсутствии связи с СЕРВЕРОМ
* LimitMessage – сообщение, когда СЕРВЕР исчерпал свой лимит аккаунтов.
* ReleasedMessage – сообщение перед завершением программы, когда аккаунт освобожден.
* IpAddress – адрес СЕРВЕРА в локальной вычислительной сети
* ProcessName – название отслеживаемого процесса. Не рекомендуется изменять его
